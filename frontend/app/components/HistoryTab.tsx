      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-2">
            <div
              className={`p-1.5 rounded-lg ${isOsm ? "bg-blue-50 text-blue-600" : "bg-slate-100 text-slate-500"}`}
            >
              {isOsm ? (
            ...
            <div>
              <div className="text-[13px] font-medium text-slate-700">
                {isOsm
                  ? "OSM Real Map"
                  : `Grid ${item.grid_size}×${item.grid_size}`}
              </div>
              {astar?.heuristic && (
          ...
          <div className="flex flex-col gap-1.5">
            {dijk && (
              <div className="flex items-center gap-2 text-[13px]">
                <Badge
                  variant="outline"
                  className="border-blue-200 text-blue-600 bg-blue-50 text-sm py-0 px-1.5 font-semibold"
                >
                  Dijkstra
                </Badge>
                <span className="font-mono text-slate-600 font-medium">
                  {formatMs(dijk.execution_time || 0)}
                </span>
                <span className="text-slate-400">•</span>
                <span className="text-slate-500">
                  {(dijk.nodes_visited || 0).toLocaleString()} nodes
                </span>
              </div>
            )}
            {astar && (
              <div className="flex items-center gap-2 text-[13px]">
                <Badge
                  variant="outline"
                  className="border-cyan-200 text-cyan-600 bg-cyan-50 text-sm py-0 px-1.5 font-semibold"
                >
                  A*
                </Badge>
                <span className="font-mono text-slate-600 font-medium">
                  {formatMs(astar.execution_time || 0)}
                </span>
                <span className="text-slate-400">•</span>
                <span className="text-slate-500">
                  {(astar.nodes_visited || 0).toLocaleString()} nodes
                </span>
              </div>
            )}
          </div>
    ...
    <div className="space-y-6 max-w-7xl mx-auto w-full">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 mb-0.5">
            <div className="w-1 h-5 rounded-full bg-linear-to-b from-violet-400 to-purple-500" />
            ...
            emptyMessage={
              <div className="py-12 text-center">
                ...
                <p className="text-sm font-semibold text-slate-400">
                  Chưa có lịch sử nào được ghi lại
                </p>
                <p className="text-sm text-slate-300 mt-1">
                  Chạy thực nghiệm ở tab So Sánh để bắt đầu
                </p>
              </div>
            }
          />
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-slate-500 font-medium">
          Hiển thị {(validPage - 1) * itemsPerPage + 1} -{" "}
          {Math.min(validPage * itemsPerPage, historyList.length)} trong tổng số{" "}
